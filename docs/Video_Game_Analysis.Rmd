...
top_20_max_rev<-df%>%group_by(Name)%>%summarise(total_sale=sum(Global_Sales))%>%top_n(10)%>%arrange(desc(total_sale))
...
```{r}
ggplot(data=top_20_max_rev,aes(x=Name,y=total_sale,color=total_sale))+geom_point()+coord_flip()
```

### Top 5 Games In a Region
;fire in the eye are to make things and to let hings are we are to make it and 
